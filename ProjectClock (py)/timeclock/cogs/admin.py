...
class Admin(commands.Cog):
    ...
    async def cog_slash_command_error(
        ...
        if isinstance(error, commands.CheckFailure):
            return await inter.response.send_message(
                f"يبدو أنه ليس لديك صلاحية لاستخدام هذا الأمر",
                ephemeral=True,
            )
    ...
    async def config_edit_embed(
        ...
        if not self.check_channel_permissions(inter):
            ...
            return await inter.response.send_message(
                "الرجاء التأكد من تفعيل الصلاحيات التالية في هذه القناة قبل استخدام هذا الأمر.\n"
                "`إرسال الرسائل`, `عرض القناة`, `قراءة سجل الرسائل`",
                ephemeral=True,
            )
    ...
    async def config_view_roles(self, inter: disnake.GuildCommandInteraction) -> None:
        ...
        if roles:
            ...
            description = "لم يتم تكوين أي أدوار حتى الآن."

        embed = disnake.Embed(title="الأدوار المكونة", description=description)
    ...
    async def config_remove_role(self, inter: disnake.GuildCommandInteraction, role: str):
        ...
        await inter.response.send_message(f"تم حذف الدور!", ephemeral=True)
